...
import { History, Pizza, ShoppingCart } from "lucide-react";
import { useState } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
type PageView = "home" | "orders" | "cart";
const NavMenu = [
  {
    name: "Trang chủ",
    link: "/",
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-muted-foreground">
            ...
            {isAuthenticated ? (
              <button
                onClick={() => {}}
                className={`transition-colors text-[14px] flex items-center gap-1 ${pageView === "orders" ? "text-primary" : ""}`}
              >
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => {}} className="relative p-2 rounded-lg hover:bg-muted transition-colors">
